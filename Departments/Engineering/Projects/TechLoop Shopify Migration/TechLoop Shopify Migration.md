...
| 12    | Workflow Rebuild Tier 4 — Alerts/Utilities | ✅ Done (2026-06-18) | **TL_Price_Watchdog** (`yExNIbiIGVFOoJ4N`) rebuilt for Shopify: `vw_low_margin_products` re-pointed to Shopify mirror tables (`tl_shopify_products_mirror` + `tl_shopify_variants_mirror`); Slack alert now shows "Edit in Shopify" link + one-click "Set to RRP" interactive button (routes through `TL_Slack_Interaction_Handler_Shopify` `set_price_to_rrp` case → GraphQL `productVariantsBulkUpdate`). **TL_Queue_Reviewer_Shopify** (`20i0wyIaAnu9h2Wh`) already active since 2026-05-31. |
